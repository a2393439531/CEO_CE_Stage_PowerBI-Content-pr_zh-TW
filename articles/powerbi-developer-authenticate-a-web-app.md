...
|設定|描述|值|
|:--|:--|:--|
|用戶端識別碼|Client ID is used by the application to identify themselves to the users that they are requesting permissions from.|To get a Power BI app client id, see <bpt id="p1">[</bpt>How to get a client app id<ept id="p1">](powerbi-developer-register-a-web-app.md#clientID)</ept>.|
|用戶端機密|The client secret key is sent along with a Client ID when authenticating to Azure AD to call a web API.|To get a Power BI app client secret key, see <bpt id="p1">[</bpt>How to get a client secret key<ept id="p1">](powerbi-developer-register-a-web-app.md#clientSecret)</ept>.|
